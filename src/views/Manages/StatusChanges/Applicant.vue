...
export default Vue.extend({
  ...
  created() {
    axios.get(`http://localhost:8888/api/v1/apply_job/?job_id=${ this.id }&apply_status_id=${ m.APPLY_STATUS_APPLY  }`)
    .then(response => {
      ...
      <v-row>
        ...
        <v-sheet class="manage">
          <v-row class="manage__header">
            <router-link :to="`/manage/applicant/${ id }`" class="router-link-active-click">
              <span>応募者</span> 
            </router-link>
